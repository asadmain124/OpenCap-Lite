...
export default async function ScenarioDetailPage({
  ...
  const round: PayloadRound = ri
    ? {
        ...
        pricedRoundPricePerShareOverride:
          ri.pricedRoundPricePerShareOverride?.toString() ?? null,
        roundCloseDate: (ri.roundCloseDate ?? new Date())
          .toISOString()
          .slice(0, 10),
        optionPoolTopUpMode:
          ri.optionPoolTopUpMode as PayloadRound["optionPoolTopUpMode"],
        ...
        optionPoolFixedPercentPreMoney:
          ri.optionPoolFixedPercentPreMoney?.toString() ?? null,
        capDenominatorMethod:
          ri.capDenominatorMethod as PayloadRound["capDenominatorMethod"],
        capDenominatorOverride: ri.capDenominatorOverride?.toString() ?? null,
        preMoneyDenominatorMethod:
          ri.preMoneyDenominatorMethod as PayloadRound["preMoneyDenominatorMethod"],
        preMoneyDenominatorOverride:
          ri.preMoneyDenominatorOverride?.toString() ?? null,
        conversionOrderingRule:
          ri.conversionOrderingRule as PayloadRound["conversionOrderingRule"],
        notesConvertUsing:
          ri.notesConvertUsing as PayloadRound["notesConvertUsing"],
        safesConvertUsing:
          ri.safesConvertUsing as PayloadRound["safesConvertUsing"],
  ...
  const newInstruments: InitialScenarioState["newInstruments"] =
    scenario.newInstrumentInputs.map((i) => {
      ...
      return {
        ...
        annualInterestRatePercent: notes.annualInterestRatePercent as
          | string
          | undefined,
        interestType: notes.interestType as "SIMPLE" | "COMPOUND" | undefined,
        compoundingFrequencyPerYear: notes.compoundingFrequencyPerYear as
          | number
          | null
          | undefined,
      };
