...
type Step = "founders" | "pool" | "investors" | "review";
...
export function OnboardingWizard({ companyId }: { companyId: string }) {
  ...
  const [founders, setFounders] = React.useState<FounderRow[]>([
    { id: rowId(), name: "Founder 1", shares: "8000000" },
  ]);
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Add each founder and how many common shares they hold. We&rsquo;ll create
        a Common Stock class automatically.
      </p>
      {founders.map((f) => (
        <div key={f.id} className="grid grid-cols-[1fr,160px,40px] gap-2">
      ...
      <Button
        ...
        onClick={() =>
          setFounders((prev) => [
            ...prev,
            { id: rowId(), name: `Founder ${prev.length + 1}`, shares: "" },
          ])
        }
      >
        <Plus className="mr-1 h-4 w-4" /> Add founder
      </Button>
    </div>
  );
}
...
function InvestorsStep({
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Add any existing investors who hold shares (e.g. a prior priced round).
        Skip if you only have founders or your investors are on SAFEs/notes —
        you can add SAFEs from the Cap Table sidebar after this.
      </p>
      {investors.map((inv) => (
        <div
          key={inv.id}
  ...
  return (
    <div className="space-y-3 text-sm">
      <p className="text-muted-foreground">
        We&rsquo;ll create the security classes, stakeholders, and holdings
        below. You can edit anything afterwards.
      </p>
      <ul className="space-y-1 rounded-md border bg-muted/30 p-3 text-xs">
        <li>
          <span className="font-medium">{founders.length} founder(s)</span> —{" "}
          {fmt(founderTotal)} common shares
        </li>
        <li>
        ...
        <li>
          <span className="font-medium">{investors.length} investor(s)</span> —{" "}
          {fmt(investorTotal)} shares
        </li>
        <li className="border-t pt-1 font-medium">
          Initial fully-diluted ≈ {fmt(fd)}
        </li>
      </ul>
    </div>
  );
